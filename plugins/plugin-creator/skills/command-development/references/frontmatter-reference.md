...
```yaml
allowed-tools: Task                    # Launch subagents
allowed-tools: Task(agent-name)        # Only specific agent
```

**Skill tool (for loading skills):**
```yaml
allowed-tools: Skill                   # Load any skill
allowed-tools: Skill(skill-name)       # Only specific skill
```

**Orchestration command (agents + skills):**
```yaml
