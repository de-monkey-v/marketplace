  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Command: $ARGUMENTS.command. Analyze for: 1) Destructive operations (rm -rf, dd, mkfs, etc) 2) Privilege escalation (sudo) 3) Network operations without user consent. Return 'approve' or 'deny' with explanation.",
          "timeout": 15
        }
      ]
    }
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "File path: $ARGUMENTS.file_path. Content preview: $ARGUMENTS.content (first 200 chars). Verify: 1) Not system directories (/etc, /sys, /usr) 2) Not credentials (.env, tokens, secrets) 3) No path traversal 4) Content doesn't expose secrets. Return 'approve' or 'deny'."
        }
      ]
    }
  ]
