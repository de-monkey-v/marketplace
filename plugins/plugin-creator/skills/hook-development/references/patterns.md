  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "File path: $ARGUMENTS.file_path. Verify: 1) Not in /etc or system directories 2) Not .env or credentials 3) Path doesn't contain '..' traversal. Return 'approve' or 'deny'."
        }
      ]
    }
  ]
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Command: $ARGUMENTS.command. If command contains 'rm', 'delete', 'drop', or other destructive operations, return 'ask' to confirm with user. Otherwise 'approve'."
        }
      ]
    }
  ]
...
## Pattern 7.5: PermissionRequest Auto-Handling

Automatically approve safe operations via PermissionRequest hooks:
  ...
  "PermissionRequest": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CLAUDE_PLUGIN_ROOT}/scripts/auto-approve-read.sh",
          "timeout": 5
        }
      ]
    }
...
**Example script (auto-approve-read.sh):**
```bash
#!/bin/bash
input=$(cat)
file_path=$(echo "$input" | jq -r '.tool_input.file_path // .tool_input.path // ""')

# Auto-approve reads within project directory
if [[ "$file_path" == "$CLAUDE_PROJECT_DIR"* ]]; then
  echo '{"hookSpecificOutput": {"hookEventName": "PermissionRequest", "permissionDecision": "allow", "permissionDecisionReason": "Read within project"}}'
  exit 0
fi

# Ask user for reads outside project
echo '{"hookSpecificOutput": {"hookEventName": "PermissionRequest", "permissionDecision": "ask"}}'
```

**Use for:** Streamlining workflows by auto-approving safe operations.
