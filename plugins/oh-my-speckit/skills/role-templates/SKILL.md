...
| 역할 | 에이전트 타입 | 권장 모델 | 특성 | 도구 |
|------|-------------|----------|------|------|
| pm | `claude-team:planner` | **opus** | 제품 기획/요구사항 분석 (읽기 전용 + 웹) | Read,Glob,Grep,Bash,WebSearch,WebFetch,SendMessage |
| architect | `claude-team:architect` | **opus** | 아키텍처 분석/설계 (읽기 전용) | Read,Glob,Grep,Bash,SendMessage |
| developer | `claude-team:implementer` | sonnet | 코드 구현 (읽기/쓰기) | Read,Write,Edit,Glob,Grep,Bash,SendMessage |
| frontend-dev | `claude-team:frontend` | sonnet | 프론트엔드 구현 (읽기/쓰기) | Read,Write,Edit,Glob,Grep,Bash,SendMessage |
| backend-dev | `claude-team:backend` | sonnet | 백엔드 구현 (읽기/쓰기) | Read,Write,Edit,Glob,Grep,Bash,SendMessage |
| qa | `claude-team:tester` | **opus** | 테스트/검증 (읽기/쓰기) | Read,Write,Edit,Glob,Grep,Bash,SendMessage |
| critic | `claude-team:reviewer` | **opus** | 비판적 리뷰 (읽기 전용) | Read,Glob,Grep,Bash,SendMessage |
| researcher | `claude-team:researcher` | **opus** | 프로젝트 분석 (읽기 전용 + 웹) | Read,Glob,Grep,Bash,WebSearch,WebFetch,SendMessage |
| llms-analyst | `ai-cli-tools:llms` | 상속 | 외부 LLM 분석 (specify용) | Bash,Read,Glob,Grep,WebSearch,WebFetch |
| llms-advisor | `ai-cli-tools:llms` | 상속 | 외부 LLM 자문 (implement용) | Bash,Read,Glob,Grep,WebSearch,WebFetch |
| llms-reviewer | `ai-cli-tools:llms` | 상속 | 외부 LLM 코드 리뷰 (verify용) | Bash,Read,Glob,Grep,WebSearch,WebFetch |

> **모델 배정 원칙**: 분석/판단 역할(pm, qa, critic, architect, researcher) = **opus** / 코딩 역할(developer, frontend-dev, backend-dev) = sonnet / 외부 LLM 프록시(llms-*) = 상속
...
- team_name: "{team-name}"
- name: "{role-name}"
- model: "{권장 모델 참조}"  # opus / sonnet / 생략(상속)
