---
description: 기능 요청을 통합 plan.md로 생성 (Agent Teams, Claude 네이티브)
argument-hint: [기능 설명]
...
기능 요청을 분석하여 통합 plan.md(Part 1: Specification + Part 2: Implementation Plan)를 생성합니다.
...
Phase 1: 팀 구성 + 컨텍스트 수집 (팀메이트 병렬)
     ↓
Phase 1.5: 기술 스택 결정 (신규 프로젝트 시)
     ↓
Phase 2: 방향성 합의 ← 중요 결정만 질문
     ↓
Phase 3: Plan 자동 작성 (Part 1 + Part 2)
     ↓
...
3. subject: "Phase 3: Plan 작성 (Part 1 + Part 2)"
   description: "조사 결과 + 결정사항 기반 통합 plan.md 자동 작성"
   activeForm: "plan.md 작성 중"
...
### Step 2: plan.md Part 1 작성 (Specification)

Phase 1 팀메이트 결과 + Phase 2 결정사항을 기반으로 plan.md Part 1을 메모리에 작성:
...
### Step 3: plan.md Part 2 작성 (Implementation Plan)

Part 1 + architect 분석결과를 기반으로 plan.md Part 2를 메모리에 작성:
...
```
AskUserQuestion:
- question: "Plan을 저장할까요?"
- header: "최종 승인"
- options:
  - label: "저장 (권장)"
    description: ".specify/specs/{id}/에 plan.md 저장"
  - label: "전체 내용 확인 후 저장"
...
- content: [완성된 plan.md 내용 (Part 1 + Part 2)]
```

#### "전체 내용 확인 후 저장" 선택 시

전체 plan.md 내용을 표시한 후 저장 여부 재확인.
...
2. AskUserQuestion: "기존 문서가 있습니다. 어떻게 하시겠어요?"
   - "처음부터 다시"
   - "기존 Part 1 (Specification) 수정"
   - "기존 Part 2 (Implementation Plan) 수정"
   - "기존 plan 검토만"
3. 선택에 따라 진행
...
- `skills/plan-writing/references/plan-template.md` (Part 1 + Part 2 통합)
